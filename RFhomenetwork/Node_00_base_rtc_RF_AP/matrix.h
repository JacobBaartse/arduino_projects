...
void loadsequencepicture(unsigned long actualtime){
  static unsigned long runningtime = 0;
  static int pointer = 0;
  if (actualtime < runningtime) return;
  loadstaticpicture(sequencing[pointer][0]);
  runningtime = (unsigned long) actualtime + sequencing[pointer][1];
...
bool alarmingsequence(unsigned long actualtime){
  static unsigned long alarmtime = 0;
  static int alarmcount = 0;
  static bool fullred = false;
  if (actualtime < alarmtime) return true;
  fullred = !fullred;
  displayFull(fullred);
  alarmcount++;
  alarmtime = (unsigned long) actualtime + 500;
