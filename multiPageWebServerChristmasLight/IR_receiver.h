...
volatile unsigned long _ir = 0;
int prev_trace_index = 0;
unsigned long prv_rfcommand = 0;
const bool debug_ir = false;
...
unsigned long getIrCode(){
  ...
  while (decoding){
    ...
    if (trace_index != prev_trace_index){
      if (debug_ir){ Serial.print("trace_index");    Serial.println(trace_index); }
      if (debug_ir){ Serial.print("prev_trace_index");    Serial.println(prev_trace_index); }
    }
    ...
    for (int index=prev_trace_index; index<tmp_trace_index; index++){
      if (debug_ir){ Serial.print(trace_array[index%buffer_size]);       Serial.print(" "); }
      prev_trace_index = index % buffer_size;
      if (trace_array[index%buffer_size] == start_indicator_val){
        start_indicator_found = true; 
        if (debug_ir) Serial.println();
        if (debug_ir) Serial.println("start_indicator_found");
        break;       
      }
    }
    ...
    if (start_indicator_found){
      if (debug_ir) Serial.println("decode the data");
      ...
      if (tmp_trace_index > prev_trace_index + 12){ // is there enough data to decode?
        if (debug_ir) Serial.println("enough data to decode");
        ...
        for (int index = prev_trace_index+1; index < prev_trace_index + 13; index ++){  // skip the start indicator
          ...
          if (debug_ir) Serial.print(bitval);

          if (bitval == start_indicator_val) {  // a new start indicator found so move forward
            if (debug_ir) Serial.println("start indicator found go to next");
          ...
          if (bit_num == 11){
            ...
            if (debug_ir) { Serial.print("0x"); Serial.println(decoded_value, HEX); }
            prev_trace_index = (index + 1) % buffer_size;
...
unsigned long prev_micros_ir=0;
void logRfTime(){
  /*
  ...
  */
  ...
  int elapsed_micros = (cur_micros - prev_micros_ir);
